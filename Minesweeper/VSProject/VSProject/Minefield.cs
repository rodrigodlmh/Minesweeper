...
namespace VSProject
{
    ...
    {
        /// <summary>
        /// Variable that holds the amount of columns the grid is going to have on the medium difficulty
        /// </summary>
        public int Columns;

        /// <summary>
        /// Variable that holds the amount of rows the grid is going to have on the medium difficulty
        /// </summary>
        public int Rows;
        ...
        public void GenerateMines()
        {
            ...
            while (i < this.MineCount)
            {
                ...
                Coordinate mineCoord;

                // Randomly generate the X and Y components of that coordinate
                mineCoord = this.GenerateRandomCoordinate();
                ...
                {
                    if ((mineCoord.X == coord.X) && (mineCoord.Y == coord.Y))
                    {
        ...
        public Coordinate GenerateRandomCoordinate()
        {
            ...
            c.X = this.Random.Next(0, this.Columns);
            c.Y = this.Random.Next(0, this.Rows);
            return c;
        }
        ...
        public void SetStateOfSquares()
        {
            ...
            foreach (Coordinate mine in mines)
            {
                ...
                if (this.IsCoordinateValid(mine))
                {
                    // Only chenge state of square if it isn't a mine or already has 8 mines around it
                    if (this.Squares[mine.X, mine.Y].State != State.EightMines && this.Squares[mine.X, mine.Y].State != State.IsAMine)
                    {
        ...
        {
            // get coordinates of first click on the game board
            int x = firstClick.X;
            int y = firstClick.Y;
        ...
        /// <summary>
        /// get coordinates around a square, 8 total 
        /// </summary>
        ...
        /// <summary>
        /// checks if coordinate is valid and available 
        /// </summary>
        /// <param name="coordinate"> a coordinate</param>
        /// <returns>A boolean</returns>
        public bool IsCoordinateValid(Coordinate coordinate)
        {
            if (coordinate.X < this.Columns && coordinate.X >= 0 && coordinate.Y < this.Rows && coordinate.Y >= 0)
            {
                return true;
            }
            else
